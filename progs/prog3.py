...
def prog3(fullpathtoxlsxfile):
    ...
    for i in range(0, len(a)):
        for j in a[i][1]:
            ...
            print(f"В файл {(table_for_act[-1][1]).date().month:02d}_Акты_{a[i][0]}_{(table_for_act[-1][1]).date().year}.docx добавлен акт №{table_for_act[j][5]} от {table_for_act[j][1].date()}")
            docum.save(f"{actdir}\\{(table_for_act[-1][1]).date().month:02d}_Акты_{a[i][0]}_{(table_for_act[-1][1]).date().year}.docx")
            ...
            docum.save(f"{actdir}\\{(table_for_act[-1][1]).date().month:02d}_Акты_{a[i][0]}_{(table_for_act[-1][1]).date().year}.docx")
            docum = DocxTemplate(f"{actdir}\\{(table_for_act[-1][1]).date().month:02d}_Акты_{a[i][0]}_{(table_for_act[-1][1]).date().year}.docx")
        docum = DocxTemplate(r"..\konfs\Шаблоны\Шаблон.docx")
...
if __name__ == '__main__':
    prog3(r"..\2_Журнал_и_акты\Журнал_НШЛ.xlsx")
